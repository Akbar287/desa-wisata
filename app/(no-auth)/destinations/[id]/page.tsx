...
export default async function DestinationDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const dest = await prisma.destination.findUnique({
        where: { id: destId },
        include: {
            destinationLabels: { include: { label: { select: { id: true, name: true, icon: true } } } },
            destinationAccessibilities: { include: { accessibility: { select: { id: true, name: true, icon: true, description: true } } } },
            destinationFacilities: { include: { facility: { select: { id: true, name: true, icon: true, description: true } } } },
    ...
    const relatedDests = labelIds.length > 0 ? await prisma.destination.findMany({
        where: {
            id: { not: dest.id },
            isAktif: true,
        ...
        take: 3,
        orderBy: { rating: 'desc' },
        select: { id: true, name: true, imageBanner: true, priceWeekday: true, rating: true, jamBuka: true, jamTutup: true },
    }) : [];
    ...
    const serialized = {
        ...
        labels: dest.destinationLabels.map(dl => ({ name: dl.label.name, icon: dl.label.icon })),
        accessibilities: dest.destinationAccessibilities.map(da => ({ name: da.accessibility.name, icon: da.accessibility.icon, description: da.accessibility.description })),
        facilities: dest.destinationFacilities.map(df => ({ name: df.facility.name, icon: df.facility.icon, description: df.facility.description })),
    ...
    return <DestinationDetailComponents destination={serialized} relatedDestinations={relatedSerialized} />;
}
