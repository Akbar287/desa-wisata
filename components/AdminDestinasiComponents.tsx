...
import {
  destinationFullSchema,
  galleryDestinationSchema,
  mapMasterSchema,
  masterItemSchema,
...
function ImageUploadField({
  ...
  return (
    <div className="space-y-2">
      ...
      {value && (
        <img src={value} alt="" className="h-16 rounded border object-cover" />
      )}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mt-4">
      <p className="text-sm text-muted-foreground">
        Menampilkan {offset + 1}–{Math.min(offset + limit, pagination.total)}{" "}
...
export default function AdminDestinasiComponents({
  ...
  const refreshDetail = () => {
    if (selectedDest) fetchDetail(selectedDest.id);
  };
  ...
  const fetchRefData = useCallback(async () => {
    ...
    if (json.status === "success") {
      setRefData({
        ...
        accessibilities: json.data?.accessibilities ?? [],
  ...
  const onDestSubmit = async (data: any) => {
    ...
    try {
      ...
      if (json.status === "success") {
        ...
        fetchList();
        if (editingDestId && selectedDest) refreshDetail();
      } else toast.error(json.message);
    } catch {
      toast.error("Gagal");
    } finally {
  ...
  const galleryForm = useForm({
    resolver: yupResolver(galleryDestinationSchema),
    defaultValues: { image: "" },
  });
  ...
  const masterForm = useForm({
    resolver: yupResolver(masterItemSchema),
  ...
  const openMapMasterAdd = () => {
    ...
    setMapMasterListOpen(false);
    setMapMasterDialog({ mode: "add" });
  };

  const openMapMasterEdit = (data: MapRefOption) => {
    mapMasterForm.reset({
      title: data.title,
    ...
    setMapMasterListOpen(false);
    setMapMasterDialog({ mode: "edit", data });
  };

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const onMapMasterSubmit = async (formData: any) => {
    if (!mapMasterDialog) return;
    setSubmitting(true);
    try {
      const isEdit = mapMasterDialog.mode === "edit";
      const editId = mapMasterDialog.data?.id;
      const payload = {
        ...formData,
  ...
  const confirmMapMasterDelete = async () => {
  ...
  const renderSection = (
    ...
    <div className="rounded-lg border p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" variant="outline" onClick={() => openSubAdd(type)}>
  ...
  const renderMapSection = () => {
    const linkedMapIds = new Set(
      (selectedDest?.maps ?? []).map((x) => x.mapId),
    );
    return (
      <div className="rounded-lg border p-4 space-y-3">
        <div className="flex items-center justify-between gap-2">
          <h4 className="font-semibold text-sm">
            Peta ({selectedDest?.maps?.length ?? 0})
          </h4>
          <Button
            size="sm"
            variant="outline"
        ...
        {refData.maps.length > 0 ? (
          <div className="space-y-2">
            {refData.maps.map((item) => {
              const checked = linkedMapIds.has(item.id);
              return (
                <div
                  key={item.id}
                  className="rounded-md border px-3 py-2 flex items-start justify-between gap-2 group"
                >
                  <label className="flex items-start gap-2 flex-1 cursor-pointer">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {item.fasilitas ? "Fasilitas" : "Non-fasilitas"} ·{" "}
                        {item.lat}, {item.lng}
                      </div>
                      {item.content && (
                  ...
                  <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button
                      ...
                      size="icon"
                      className="size-7"
                      onClick={() => openMapMasterEdit(item)}
                    >
                      <IconEdit className="size-3" />
                    </Button>
                    <Button
          ...
          <p className="text-sm text-muted-foreground">
            Belum ada map master. Klik &quot;Tambah Map Master&quot; lalu
            centang untuk menghubungkan ke destinasi ini.
          </p>
        )}
      </div>
    );
              ...
              {loading ? (
                ...
                <div className="grid gap-4">
                  ...
                  {renderMasterSection(
                    "Aksesibilitas",
                    "accessibility",
                    refData.accessibilities,
                    d.destinationAccessibilities,
                    "accessibilityId",
                    saveAccessibilities,
                  )}
      ...
      <>
        ...
        >
          <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
            <DialogHeader>
              <DialogTitle>
                {editingDestId ? "Edit Destinasi" : "Tambah Destinasi"}
              </DialogTitle>
              <DialogDescription>
                Isi informasi lengkap destinasi wisata.
              </DialogDescription>
            </DialogHeader>
            <form
              onSubmit={destForm.handleSubmit(onDestSubmit)}
              className="grid gap-4 sm:grid-cols-2"
            >
              <div className="sm:col-span-2 space-y-2">
                <Label>Nama Destinasi</Label>
                <Input {...destForm.register("name")} />
                {destForm.formState.errors.name && (
                  <p className="text-sm text-destructive">
                    {destForm.formState.errors.name.message}
                  </p>
                )}
              </div>
              <div className="space-y-2">
                <Label>Harga Weekday</Label>
                <Input type="number" {...destForm.register("priceWeekday")} />
                {destForm.formState.errors.priceWeekday && (
                  <p className="text-sm text-destructive">
                    {destForm.formState.errors.priceWeekday.message}
                  </p>
                )}
              </div>
              <div className="space-y-2">
                <Label>Harga Weekend</Label>
                <Input type="number" {...destForm.register("priceWeekend")} />
                {destForm.formState.errors.priceWeekend && (
                  <p className="text-sm text-destructive">
                    {destForm.formState.errors.priceWeekend.message}
                  </p>
                )}
              </div>
              <div className="space-y-2">
                <Label>Harga Grup</Label>
                <Input type="number" {...destForm.register("priceGroup")} />
              </div>
              <div className="space-y-2">
                <Label>Minimal Grup</Label>
                <Input type="number" {...destForm.register("minimalGroup")} />
              </div>
              <div className="space-y-2">
                <Label>Jam Buka</Label>
                <Input {...destForm.register("jamBuka")} placeholder="08:00" />
              </div>
              <div className="space-y-2">
                <Label>Jam Tutup</Label>
                <Input {...destForm.register("jamTutup")} placeholder="17:00" />
              </div>
              <div className="space-y-2">
                <Label>Durasi Rekomendasi</Label>
                <Input
                  {...destForm.register("durasiRekomendasi")}
                  placeholder="2-3 jam"
                />
              </div>
              <div className="space-y-2">
                <Label>Kuota Harian</Label>
                <Input type="number" {...destForm.register("KuotaHarian")} />
              </div>
              <div className="space-y-2">
                <Label>Rating</Label>
                <Input
                  type="number"
                  step="0.1"
                  {...destForm.register("rating")}
                />
              </div>
              <div className="space-y-2">
                <Label>Jumlah Review</Label>
                <Input type="number" {...destForm.register("reviewCount")} />
              </div>
              <div className="sm:col-span-2 flex items-center gap-2">
                <Checkbox
                  checked={destForm.watch("isAktif")}
                  onCheckedChange={(c) => destForm.setValue("isAktif", !!c)}
                />
                <Label>Aktif</Label>
              </div>
              <div className="sm:col-span-2">
                <ImageUploadField
                  value={destForm.watch("imageBanner")}
                  onChange={(v) => destForm.setValue("imageBanner", v)}
                  label="Gambar Banner"
                />
                {destForm.formState.errors.imageBanner && (
                  <p className="text-sm text-destructive">
                    {destForm.formState.errors.imageBanner.message}
                  </p>
                )}
              </div>
              <div className="sm:col-span-2 space-y-2">
                <Label>Deskripsi</Label>
                <Textarea rows={4} {...destForm.register("description")} />
                {destForm.formState.errors.description && (
                  <p className="text-sm text-destructive">
                    {destForm.formState.errors.description.message}
                  </p>
                )}
              </div>
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Semua data terkait (galeri, label, fasilitas, aksesibilitas,
                relasi peta) juga akan dihapus. Lanjutkan?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                ...
                disabled={submitting}
              >
                {submitting && <Spinner className="mr-2 size-4" />}Ya, Hapus
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
        ...
        <AlertDialog
          open={!!subDeleteDialog}
          onOpenChange={(o) => {
            if (!o) setSubDeleteDialog(null);
          }}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
        ...
        <Dialog
          open={!!subDialog}
          onOpenChange={(o) => {
            if (!o) setSubDialog(null);
          }}
        >
          <DialogContent className="max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {subDialog?.mode === "edit" ? "Edit" : "Tambah"} Gambar Galeri
              </DialogTitle>
            </DialogHeader>
            {subDialog?.type === "gallery" && (
              <form
                onSubmit={galleryForm.handleSubmit((data) =>
                  onSubSubmit("gallery", data),
                )}
                className="space-y-4"
              >
                <ImageUploadField
                  value={galleryForm.watch("image")}
                  onChange={(v: string) => galleryForm.setValue("image", v)}
                  label="Gambar"
                />
                {galleryForm.formState.errors.image && (
                  <p className="text-sm text-destructive">
                    {galleryForm.formState.errors.image.message}
                  </p>
                )}
                <DialogFooter>
                  <Button
                  ...
                  <Button type="submit" disabled={submitting}>
                    {submitting && <Spinner className="mr-2 size-4" />}Simpan
                  </Button>
                </DialogFooter>
              </form>
            )}
        ...
        <Dialog
          open={!!masterDialog}
          onOpenChange={(o) => {
            if (!o) setMasterDialog(null);
          }}
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {masterDialog?.mode === "edit" ? "Edit" : "Tambah"}{" "}
                {masterDialog ? masterLabels[masterDialog.kind] : ""}
              </DialogTitle>
              <DialogDescription>
                Isi informasi{" "}
                {masterDialog
                  ? masterLabels[masterDialog.kind].toLowerCase()
                  : ""}{" "}
                destinasi.
              </DialogDescription>
            </DialogHeader>
            <form
              onSubmit={masterForm.handleSubmit(onMasterSubmit)}
              className="space-y-4"
            >
              <div className="space-y-2">
                <Label>Nama</Label>
                <Input {...masterForm.register("name")} />
                {masterForm.formState.errors.name && (
                  <p className="text-sm text-destructive">
                    {masterForm.formState.errors.name.message}
                  </p>
                )}
              </div>
              <div className="space-y-2">
                <Label>Deskripsi</Label>
                <Textarea rows={2} {...masterForm.register("description")} />
              </div>
              <div className="space-y-2">
                <Label>Icon (emoji / class)</Label>
                <Input
                  {...masterForm.register("icon")}
                  placeholder="🏖️ atau icon-class"
                />
              </div>
              <DialogFooter>
                <Button
                ...
                <Button type="submit" disabled={submitting}>
                  {submitting && <Spinner className="mr-2 size-4" />}
                  {masterDialog?.mode === "edit" ? "Simpan" : "Tambah"}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
        ...
        <AlertDialog
          open={!!masterDeleteDialog}
          onOpenChange={(o) => {
            if (!o) setMasterDeleteDialog(null);
          }}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>
                Hapus{" "}
                {masterDeleteDialog
                  ? masterLabels[masterDeleteDialog.kind]
                  : ""}
              </AlertDialogTitle>
              <AlertDialogDescription>
                Hapus &quot;{masterDeleteDialog?.name}&quot;? Data ini juga akan
                dihapus dari semua destinasi yang terhubung.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel disabled={submitting}>Batal</AlertDialogCancel>
        ...
        <Dialog open={mapMasterListOpen} onOpenChange={setMapMasterListOpen}>
          <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Data map master dapat dipakai ulang oleh destinasi, wahana, dan
                tour.
              </DialogDescription>
            </DialogHeader>
                    ...
                    <TableBody>
                      {refData.maps.map((item) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell>
                            <span
                              className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${item.fasilitas ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-700"}`}
                            >
                              {item.fasilitas ? "Fasilitas" : "Non-fasilitas"}
                            </span>
                          </TableCell>
        ...
        {/* Map Master Form Dialog */}
        <Dialog
          open={!!mapMasterDialog}
          onOpenChange={(o) => {
            if (!o) setMapMasterDialog(null);
          }}
        >
          <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
            <DialogHeader>
              <DialogTitle>
                {mapMasterDialog?.mode === "edit" ? "Edit" : "Tambah"} Map
                Master
              </DialogTitle>
              <DialogDescription>
                Map master dapat dipakai ulang oleh banyak destinasi, wahana,
                atau tour.
              </DialogDescription>
            </DialogHeader>
            <form
            ...
            >
              <div className="sm:col-span-2 space-y-2">
                <Label>Judul</Label>
                <Input {...mapMasterForm.register("title")} />
              ...
              <div className="sm:col-span-2 space-y-2">
                ...
                {hasMapsApiKey ? (
                  <MapPointPicker
                    ...
                    lat={Number(mapMasterForm.watch("lat"))}
                    lng={Number(mapMasterForm.watch("lng"))}
                    onPick={(lat, lng) => {
                  ...
                  <div className="rounded-lg border border-dashed p-4 text-sm text-muted-foreground">
                    API key Google Maps tidak ditemukan. Isi
                    `NEXT_PUBLIC_GMAPS_API` (atau `NEXT_GMAPS_API`) untuk
              ...
              <div className="sm:col-span-2">
                <ImageUploadField
                  ...
                  onChange={(v) =>
                    mapMasterForm.setValue("image", v, {
                      shouldValidate: true,
                    })
                  }
              ...
              <div className="sm:col-span-2 space-y-2">
                <Label>Deskripsi</Label>
                <Textarea rows={3} {...mapMasterForm.register("content")} />
              </div>
              <div className="sm:col-span-2 flex items-center gap-2">
                <Checkbox
                  checked={mapMasterForm.watch("fasilitas")}
                  onCheckedChange={(c) =>
                    mapMasterForm.setValue("fasilitas", Boolean(c), {
                      shouldValidate: true,
                      shouldDirty: true,
                    })
                  }
              ...
              <DialogFooter className="sm:col-span-2">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => setMapMasterDialog(null)}
                  disabled={submitting}
                >
                  Batal
                </Button>
                <Button type="submit" disabled={submitting}>
                  {submitting && <Spinner className="mr-2 size-4" />}
                  {mapMasterDialog?.mode === "edit" ? "Simpan" : "Tambah"}
                </Button>
              </DialogFooter>
            </form>
          </DialogContent>
        ...
        {/* Map Master Delete AlertDialog */}
        <AlertDialog
          open={!!mapMasterDeleteDialog}
          onOpenChange={(o) => {
            if (!o) setMapMasterDeleteDialog(null);
          }}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Hapus map &quot;{mapMasterDeleteDialog?.title}&quot;? Relasi
                peta pada destinasi/wahana/tour akan ikut terhapus.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel disabled={submitting}>Batal</AlertDialogCancel>
              <AlertDialogAction
                variant="destructive"
                onClick={confirmMapMasterDelete}
                disabled={submitting}
              >
                {submitting && <Spinner className="mr-2 size-4" />}Ya, Hapus
              </AlertDialogAction>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  Kelola destinasi wisata beserta relasi label, aksesibilitas,
                  fasilitas, peta, dan galeri.
                </p>
              </div>
              <div className="flex items-center gap-2">
                <Select
                ...
                <Button onClick={openDestAdd} size="sm">
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Harga Weekday</TableHead>
                    ...
                    destinations.map((d, i) => {
                      ...
                      return (
                        <TableRow key={d.id}>
                          ...
                          <TableCell>
                            <span
                              className={`text-xs font-medium px-2 py-0.5 rounded-full ${d.isAktif ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                            >
