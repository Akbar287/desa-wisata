...
import {
  createAdminPaymentAvailable,
  deleteAdminPaymentAvailable,
  getAdminPaymentAvailableList,
  updateAdminPaymentAvailable,
  uploadAdminPaymentAvailableImage,
} from "@/services/AdminPaymentAvailableServices";
...
const TYPE_BADGES: PaymentTypeBadgeMap = {
...
export default function AdminPaymentAvailableComponents() {
  ...
  const [editingItem, setEditingItem] = useState<PaymentAvailableData | null>(
    null,
  );
  const [deletingItem, setDeletingItem] = useState<PaymentAvailableData | null>(
    null,
  );
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const json = await getAdminPaymentAvailableList({ page, limit, search });
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      const { url, message } = await uploadAdminPaymentAvailableImage(file);
      if (url) {
        setValue("image", url);
        toast.success("Gambar berhasil diupload");
      } else {
        toast.error(message || "Gagal upload gambar");
      }
    } catch {
  ...
  const onSubmit = async (formData: PaymentAvailableFormData) => {
    ...
    try {
      const json = editingItem
        ? await updateAdminPaymentAvailable(editingItem.id, formData)
        : await createAdminPaymentAvailable(formData);
  ...
  const handleDeleteConfirm = async () => {
    ...
    setSubmitting(true);
    try {
      const json = await deleteAdminPaymentAvailable(deletingItem.id);
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  Kelola metode pembayaran seperti Transfer Bank, E-Wallet, dan
                  QRIS.
                </p>
              </div>
                      ...
                      return (
                        <TableRow key={item.id}>
                          ...
                          <TableCell className="text-center">
                            <span
                              className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${badge.class}`}
                            >
                              {badge.label}
                            </span>
                          </TableCell>
                          <TableCell className="text-center">
                            <span
                              className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${item.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                            >
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  >
                    {uploadingImage ? (
                      <>
                        <Spinner className="mr-2 size-4" /> Mengupload...
                      </>
                    ) : (
                      "Upload Gambar"
                    )}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <label className="flex items-center gap-2 cursor-pointer">
                    <div
                      className={`relative w-10 h-5 rounded-full transition-colors ${field.value ? "bg-primary" : "bg-gray-300"}`}
            ...
            <AlertDialogDescription>
              ...
              {(deletingItem?._count?.payments ?? 0) > 0 && (
                <span className="mt-2 block text-destructive font-medium">
                  ⚠️ Metode ini memiliki {deletingItem?._count?.payments}{" "}
                  transaksi pembayaran terkait. Menghapus metode akan menghapus
