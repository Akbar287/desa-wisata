      ...
      .then((data) => {
        ...
        setGalleries(
          Array.isArray(data.galleries)
            ? data.galleries
                .map((g: { id?: unknown }) => ({ id: Number(g?.id) }))
                .filter(
                  (g: { id: number }) => Number.isInteger(g.id) && g.id > 0,
                )
            : [],
        );
      })
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
            ...
            {galleries.map((g, i) => (
              <div
                key={i}
                className="relative border rounded aspect-square overflow-hidden"
              >
                <img
            ...
            >
              <span className="text-gray-500 text-sm">Tambahkan Foto</span>
            </div>
          </div>
            ...
            try {
              ...
              await fetch("/api/profile", {
                ...
                body: JSON.stringify({
                  ...
                  visions,
                  missions,
                  galleries: finalGalleries.map((g) => ({ id: g.id })),
                }),
              });
