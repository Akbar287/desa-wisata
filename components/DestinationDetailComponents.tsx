...
interface LabelItem { name: string; icon: string }
interface AccessibilityItem { name: string; icon: string; description: string }
interface FacilityItem { name: string; icon: string; description: string }
...
function IconImg({ src, size = 20, className = '' }: { src?: string; size?: number; className?: string }) {
    if (!src) return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>;
    if (src.startsWith('data:') || src.startsWith('http')) return <img src={src} alt="" width={size} height={size} className={`inline-block ${className}`.trim()} style={{ objectFit: 'contain' }} />;
    return <span style={{ fontSize: size * 0.8 }}>{src}</span>;
}

/* ─── Component ───────────────────────────────────────── */

export default function DestinationDetailComponents({ destination: d, relatedDestinations }: {
    ...
    const navItems = [
        ...
        ...(d.facilities.length > 0 ? [{ id: 'facilities', label: 'Fasilitas' }] : []),
        ...(d.accessibilities.length > 0 ? [{ id: 'accessibility', label: 'Aksesibilitas' }] : []),
        { id: 'pricing', label: 'Harga' },
        ...(relatedDestinations.length > 0 ? [{ id: 'related', label: 'Serupa' }] : []),
    ];
    ...
    return (
        <>
            ...
            <section className="relative pt-20 min-h-[400px] md:min-h-[520px] overflow-hidden">
                {d.imageBanner ? (
                    <Image src={d.imageBanner} alt={d.name} fill className="object-cover" priority />
                ) : (
                    <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, #2D6A4F, #1B4332)' }} />
                )}
                ...
                <div className="absolute inset-0 flex items-end pb-12 md:pb-16">
                    <motion.div className="w-full max-w-[1320px] mx-auto px-6" initial="hidden" animate="visible" variants={stagger}>
                        {/* Breadcrumb */}
                        <motion.div variants={fadeUp} className="flex items-center gap-2 mb-4 font-sans text-[13px] text-white/70">
                            ...
                            <Link href="/destinations" className="text-white/70 no-underline hover:text-white transition-colors">Destinasi</Link>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="9 18 15 12 9 6" /></svg>
                        ...
                        {d.labels.length > 0 && (
                            <motion.div variants={fadeUp} className="flex flex-wrap gap-2 mb-4">
                                {d.labels.map(l => (
                                    <span key={l.name} className="px-4 py-1.5 rounded-full text-xs font-sans font-semibold text-white bg-white/15 backdrop-blur-sm border border-white/20 tracking-wide flex items-center gap-1.5">
                                        {l.icon && <IconImg src={l.icon} size={14} />}{l.name}
                                    </span>
                                ))}
                            </motion.div>
                        )}

                        <motion.h1 variants={fadeUp} className="font-serif font-extrabold text-white mb-4 max-w-[680px]" style={{ fontSize: 'clamp(26px, 4.5vw, 48px)', lineHeight: 1.15, textShadow: '0 4px 30px rgba(0,0,0,0.3)' }}>
                            {d.name}
                        </motion.h1>
                        ...
                        <motion.div variants={fadeUp} className="flex flex-wrap gap-4 mt-2">
                            {[
                                ...
                                { emoji: '⭐', value: `${d.rating.toFixed(1)} (${d.reviewCount} ulasan)` },
                ...
                <div className="max-w-[1320px] mx-auto px-3 flex gap-0">
                    {navItems.map(n => (
                        <button
                            ...
                            style={{ color: activeSection === n.id ? 'var(--color-primary)' : 'var(--color-text-muted)', fontWeight: activeSection === n.id ? 700 : 500 }}
                        >
                            {n.label}
                            {activeSection === n.id && (
                                <motion.div layoutId="activeDestNavTab" className="absolute bottom-0 left-0 right-0 h-[3px] rounded-t-full" style={{ background: 'linear-gradient(90deg, var(--color-primary), #52B788)' }} transition={{ type: 'spring', bounce: 0.2, duration: 0.5 }} />
                            )}
                        </button>
                    ))}
                </div>
            ...
            <main className="max-w-[1320px] mx-auto px-6 py-12" style={{ background: 'var(--color-bg)' }}>
                <div className="tour-detail-layout flex gap-12 items-start flex-col lg:flex-row">
                    ...
                    <div className="flex-1 min-w-0 w-full">
                        ...
                        {/* Overview */}
                        <motion.section id="overview" className="mb-14" initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={stagger}>
                            <motion.div variants={fadeUp} className="flex flex-wrap gap-5 mb-7">
                                {[
                                    { icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z', label: 'Jam Operasi', value: `${d.jamBuka} – ${d.jamTutup}` },
                                    { icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z', label: 'Durasi Rekomendasi', value: d.durasiRekomendasi },
                                    { icon: 'M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8zm14 10v-2a3 3 0 00-2.12-2.87M16 3.13a3 3 0 010 5.74', label: 'Kuota Harian', value: `${d.KuotaHarian} orang` },
                                ].map((f, i) => (
                            ...
                            <motion.p variants={fadeUp} className="text-[15px] leading-relaxed font-sans mb-6" style={{ color: 'var(--color-text-light)', lineHeight: 1.8 }}>
                                {d.description || 'Destinasi wisata yang menarik dan layak untuk dikunjungi.'}
                            </motion.p>

                            {/* Labels highlight */}
                            {d.labels.length > 0 && (
                                <motion.div variants={fadeUp} className="rounded-2xl p-6" style={{ background: 'var(--color-cream)', border: '1px solid var(--color-border-subtle)' }}>
                                    <h4 className="font-serif text-base font-bold mb-4" style={{ color: 'var(--color-text)' }}>Label Destinasi</h4>
                                    <ul className="list-none p-0 m-0 grid gap-2.5" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))' }}>
                                        {d.labels.map(l => (
                                            <li key={l.name} className="flex items-center gap-2.5 font-sans text-sm" style={{ color: 'var(--color-text-light)' }}>
                        ...
                        {/* Gallery */}
                        {d.gallery.length > 0 && (
                            <motion.section id="gallery" className="mb-14" initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={stagger}>
                        ...
                        {d.facilities.length > 0 && (
                            <motion.section id="facilities" className="mb-14" initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={stagger}>
                                <motion.h2 variants={fadeUp} className="font-serif font-bold mb-6" style={{ fontSize: 'clamp(22px, 3vw, 28px)', color: 'var(--color-text)' }}>
                                    Fasilitas
                                </motion.h2>
                                    ...
                                    {d.facilities.map((f, i) => (
                                        ...
                                        >
                                            <div className="flex items-center gap-3 mb-2">
                                                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: 'var(--color-cream)' }}>
                                                    <IconImg src={f.icon || undefined} size={22} />
                                                </div>
                                                <h4 className="font-sans text-[15px] font-bold" style={{ color: 'var(--color-text)' }}>{f.name}</h4>
                                            </div>
                                            {f.description && (
                                                <p className="font-sans text-sm leading-relaxed m-0" style={{ color: 'var(--color-text-muted)' }}>{f.description}</p>
                                            )}
                                        </motion.div>
                                    ))}
                                </motion.div>
                        ...
                        {d.accessibilities.length > 0 && (
                            <motion.section id="accessibility" className="mb-14" initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={stagger}>
                                <motion.h2 variants={fadeUp} className="font-serif font-bold mb-6" style={{ fontSize: 'clamp(22px, 3vw, 28px)', color: 'var(--color-text)' }}>
                                    Aksesibilitas
                                </motion.h2>
                                <motion.div variants={fadeUp} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {d.accessibilities.map((a, i) => (
                                        <motion.div key={i} variants={fadeUp} className="rounded-2xl p-5 border-l-4 transition-shadow duration-300 hover:shadow-md" style={{ background: 'var(--color-cream)', borderLeftColor: 'var(--color-primary)', border: '1px solid var(--color-border-subtle)', borderLeft: '4px solid var(--color-primary)' }}>
                                            <div className="flex items-center gap-2 mb-2">
                                                <span className="flex items-center justify-center w-7 h-7"><IconImg src={a.icon || undefined} size={22} /></span>
                                                <h4 className="font-sans text-[15px] font-bold" style={{ color: 'var(--color-text)' }}>{a.name}</h4>
                                            </div>
                                            {a.description && (
                                                <p className="font-sans text-sm leading-relaxed m-0" style={{ color: 'var(--color-text-light)', lineHeight: 1.7 }}>{a.description}</p>
                                            )}
                                        </motion.div>
                                    ))}
                                </motion.div>
                        ...
                        {/* Pricing */}
                        <motion.section id="pricing" className="mb-14" initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={stagger}>
                            <motion.h2 variants={fadeUp} className="font-serif font-bold mb-6" style={{ fontSize: 'clamp(22px, 3vw, 28px)', color: 'var(--color-text)' }}>
                                Informasi Harga
                            </motion.h2>
                                ...
                                <table className="w-full font-sans text-sm" style={{ borderCollapse: 'collapse' }}>
                                    <thead>
                                        <tr style={{ background: 'var(--color-cream)' }}>
                                            <th className="text-left py-3.5 px-5 font-bold text-xs uppercase tracking-wider" style={{ color: 'var(--color-text)' }}>Jenis Tiket</th>
                                            <th className="text-right py-3.5 px-5 font-bold text-xs uppercase tracking-wider" style={{ color: 'var(--color-text)' }}>Harga</th>
                                            <th className="text-left py-3.5 px-5 font-bold text-xs uppercase tracking-wider" style={{ color: 'var(--color-text)' }}>Keterangan</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {[
                                            { type: 'Weekday (Senin–Jumat)', price: d.priceWeekday, note: 'Harga per orang', color: '#10B981' },
                                            { type: 'Weekend (Sabtu–Minggu)', price: d.priceWeekend, note: 'Harga per orang', color: '#F59E0B' },
                                            { type: `Grup (min. ${d.minimalGroup} orang)`, price: d.priceGroup, note: `Minimal ${d.minimalGroup} orang`, color: '#6366F1' },
                                        ].map((item, i) => (
                                            <tr key={i} className="group transition-colors duration-200 hover:bg-(--color-cream)" style={{ borderBottom: '1px solid var(--color-border-subtle)' }}>
                                                <td className="py-3.5 px-5">
                                                    <span className="flex items-center gap-2">
                                                        ...
                                                        <span className="font-medium" style={{ color: 'var(--color-text)' }}>{item.type}</span>
                                                    </span>
                                                </td>
                                                <td className="py-3.5 px-5 text-right font-bold" style={{ color: 'var(--color-primary)' }}>{fmt(item.price)}</td>
                                                <td className="py-3.5 px-5" style={{ color: 'var(--color-text-light)' }}>{item.note}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            <motion.div variants={fadeUp} className="flex flex-col gap-3 md:hidden">
                                {[
                                    { type: 'Weekday', desc: 'Senin – Jumat', price: d.priceWeekday, color: '#10B981' },
                                    { type: 'Weekend', desc: 'Sabtu – Minggu', price: d.priceWeekend, color: '#F59E0B' },
                                    { type: 'Grup', desc: `Min. ${d.minimalGroup} orang`, price: d.priceGroup, color: '#6366F1' },
                                ].map((item, i) => (
                                    <div key={i} className="rounded-2xl p-5 flex items-center justify-between gap-3 transition-shadow duration-300 hover:shadow-lg" style={{ background: 'var(--color-white)', border: '1px solid var(--color-border-subtle)' }}>
                                        <div className="flex items-center gap-3">
                                            <span className="w-3 h-3 rounded-full shrink-0" style={{ background: item.color }} />
                                            <div>
                                                <div className="font-sans text-sm font-bold" style={{ color: 'var(--color-text)' }}>{item.type}</div>
                                                <div className="font-sans text-xs" style={{ color: 'var(--color-text-muted)' }}>{item.desc}</div>
                                            </div>
                                        </div>
                                        <span className="font-sans text-lg font-extrabold whitespace-nowrap" style={{ color: 'var(--color-primary)' }}>{fmt(item.price)}</span>
                                    </div>
                                ))}
                            </motion.div>
                        </motion.section>
                        ...
                        {relatedDestinations.length > 0 && (
                            <motion.section id="related" className="mb-5" initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={stagger}>
                                <motion.h2 variants={fadeUp} className="font-serif font-bold mb-6" style={{ fontSize: 'clamp(22px, 3vw, 28px)', color: 'var(--color-text)' }}>
                                    Destinasi Serupa
                                </motion.h2>
                                <motion.div variants={fadeUp} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                                    {relatedDestinations.map(rd => (
                                        <Link key={rd.id} href={`/destinations/${rd.id}`} className="no-underline group">
                                            <div className="rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lg" style={{ background: 'var(--color-white)', border: '1px solid var(--color-border-subtle)' }}>
                                                <div className="relative h-[160px] overflow-hidden">
                                                    {rd.imageBanner ? (
                                                        <Image src={rd.imageBanner} alt={rd.name} fill className="object-cover transition-transform duration-500 group-hover:scale-110" />
                                                    ) : (
                                                        <div className="w-full h-full flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #2D6A4F20, #52B78830)' }}>
                                                ...
                                                <div className="p-4">
                                                    <h4 className="font-serif text-[15px] font-bold mb-2 leading-snug transition-colors duration-300 group-hover:text-(--color-primary)" style={{ color: 'var(--color-text)' }}>{rd.name}</h4>
                                                    <div className="font-sans text-[13px]" style={{ color: 'var(--color-text-muted)' }}>
                                                        {rd.jamBuka}–{rd.jamTutup} · mulai {fmt(rd.priceWeekday)}
                                                    </div>
                                                </div>
                                            </div>
                                        </Link>
                    ...
                    <aside className="tour-detail-sidebar hidden lg:block w-[310px] shrink-0 sticky top-[120px] pt-2">
                        ...
                        >
                            ...
                            <div className="text-3xl font-extrabold font-sans mb-1" style={{ color: 'var(--color-primary)' }}>{fmt(d.priceWeekday)}</div>
                            <div className="text-[13px] font-sans mb-6" style={{ color: 'var(--color-text-muted)' }}>per orang (weekday)</div>
                            ...
                            <button className="btn-outline w-full justify-center py-3 px-6 mb-6 text-sm" onClick={() => scrollTo('pricing')}>Lihat Harga</button>
                            ...
                            <div className="flex flex-col gap-3.5 pt-5" style={{ borderTop: '1px solid var(--color-border-subtle)' }}>
                                {[
                                    ...
                                    { label: 'Rating', value: `⭐ ${d.rating.toFixed(1)} (${d.reviewCount})` },
                                    { label: 'Harga Weekend', value: fmt(d.priceWeekend) },
                                    { label: 'Harga Grup', value: `${fmt(d.priceGroup)} (min ${d.minimalGroup})` },
                                ].map(f => (
                                    <div key={f.label} className="flex justify-between font-sans text-sm">
                                        <span style={{ color: 'var(--color-text-muted)' }}>{f.label}</span>
                                        <span className="font-semibold text-right" style={{ color: 'var(--color-text)' }}>{f.value}</span>
                                    </div>
                                ))}
                            </div>
                        </motion.div>
            ...
            >
                <div>
                    ...
                    <div className="font-sans text-xl font-extrabold" style={{ color: 'var(--color-primary)' }}>{fmt(d.priceWeekday)}</div>
                </div>
                <div className="flex gap-2">
                    <button className="btn-outline py-2.5 px-4 text-[13px]" onClick={() => scrollTo('pricing')}>Harga</button>
                    <a
                ...
                {lightboxIdx !== null && (
                    ...
                    >
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={e => e.stopPropagation()}>
                            <Image src={d.gallery[lightboxIdx]} alt="Gallery" width={1200} height={800} className="max-w-[90vw] max-h-[85vh] object-contain rounded-xl" />
                        </motion.div>
                        <motion.button
                            onClick={(e) => { e.stopPropagation(); setLightboxIdx(Math.min(d.gallery.length - 1, lightboxIdx + 1)); }}
