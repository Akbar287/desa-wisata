...
export default function PaymentComponents({
  ...
  const [step, setStep] = React.useState<"select" | "pay" | "done">(
    bookingData.payments.some((p) => p.status === "PAID")
      ? "done"
  ...
  const [currentPayment, setCurrentPayment] =
    React.useState<PaymentRecord | null>(
      bookingData.payments.find((p) => p.status === "PENDING") ?? null,
    );
  const [submitting, setSubmitting] = React.useState(false);
  const [uploadingProof, setUploadingProof] = React.useState(false);
  const [syncingStatus, setSyncingStatus] = React.useState(false);
  const [proofUrl, setProofUrl] = React.useState("");
  const [paymentFlowMode, setPaymentFlowMode] = React.useState<
    "manual" | "midtrans"
  >(midtransClientKey ? "midtrans" : "manual");
  const [midtransSnapToken, setMidtransSnapToken] = React.useState<
    string | null
  >(null);
  const [midtransRedirectUrl, setMidtransRedirectUrl] = React.useState<
    string | null
  >(null);
  ...
  const grouped = paymentMethods.reduce<Record<string, PaymentMethod[]>>(
    (acc, m) => {
  ...
  React.useEffect(() => {
    ...
    if (existingScript) {
      const onLoadExisting = () => setMidtransReady(true);
      existingScript.addEventListener("load", onLoadExisting);
      if (window.snap) setMidtransReady(true);
      return () => existingScript.removeEventListener("load", onLoadExisting);
    }

    const script = document.createElement("script");
    script.src = midtransClientKey.startsWith("SB-")
  ...
  const handleSyncMidtransStatus = async (paymentId: number) => {
    ...
    if (json.status !== "success") {
      throw new Error(json.message || "Gagal sinkronisasi status Midtrans");
    }

    const payment = json.data as PaymentRecord;
  ...
  const openMidtransSnap = (snapToken: string, paymentId: number) => {
    ...
    window.snap.pay(snapToken, {
      onSuccess: async () => {
        try {
          ...
          console.error(err);
          toast(
            "Pembayaran sukses, namun gagal sinkron status. Silakan refresh.",
          );
        }
      },
      onPending: async () => {
  ...
  const launchMidtransPayment = () => {
    if (!currentPayment) return;

    if (!midtransSnapToken && midtransRedirectUrl) {
      window.location.href = midtransRedirectUrl;
      return;
    }

    if (!midtransSnapToken) {
      toast("Sesi Midtrans belum tersedia. Silakan buat pembayaran baru.");
      return;
    }

    try {
      openMidtransSnap(midtransSnapToken, currentPayment.id);
    } catch (err) {
      console.error(err);
      ...
      toast("Midtrans belum siap. Silakan ulangi beberapa saat lagi.");
    }
  };
  ...
  const handleCreatePayment = async () => {
    const resolvedPaymentMethodId =
      selectedMethodId ??
      (midtransClientKey ? (paymentMethods[0]?.id ?? null) : null);

    if (!resolvedPaymentMethodId) {
      toast("Metode pembayaran tidak tersedia.");
      return;
    }
    ...
    try {
      const res = await fetch("/api/bookings/create-payment", {
        ...
        body: JSON.stringify({
          ...
          paymentAvailableId: resolvedPaymentMethodId,
          amount: bookingData.totalPrice,
        }),
      });
      const json = await res.json();
      if (json.status !== "success") {
        toast(json.message || "Gagal membuat pembayaran");
        return;
      }

      const payment = (json.data?.payment ?? json.data) as PaymentRecord;
      const snapToken =
        typeof json.data?.snapToken === "string" ? json.data.snapToken : null;
      const redirectUrl =
        typeof json.data?.redirectUrl === "string"
          ? json.data.redirectUrl
          : null;

      const nextFlowMode =
        snapToken || redirectUrl ? ("midtrans" as const) : ("manual" as const);

      setCurrentPayment(payment);
      setPaymentFlowMode(nextFlowMode);
      setMidtransSnapToken(snapToken);
      setMidtransRedirectUrl(redirectUrl);
      setProofUrl("");
      setStep("pay");

      if (typeof window !== "undefined") {
        if (nextFlowMode === "midtrans") {
          window.sessionStorage.setItem(
            `midtrans-payment-${payment.id}`,
            JSON.stringify({
              snapToken,
              redirectUrl,
            }),
      ...
      if (snapToken && midtransReady) {
        openMidtransSnap(snapToken, payment.id);
      }
    } catch {
  ...
  React.useEffect(() => {
    if (typeof window === "undefined" || !currentPayment) return;
    if (midtransSnapToken || midtransRedirectUrl) return;
    if (!midtransClientKey) return;

    const cached = window.sessionStorage.getItem(
      `midtrans-payment-${currentPayment.id}`,
    );
    ...
    try {
      ...
      setMidtransSnapToken(parsed.snapToken ?? null);
      setMidtransRedirectUrl(parsed.redirectUrl ?? null);
    } catch {
      window.sessionStorage.removeItem(`midtrans-payment-${currentPayment.id}`);
    }
  }, [
    currentPayment,
    midtransClientKey,
  ...
  const handleCheckMidtransStatus = async () => {
    ...
    try {
      ...
      console.error(err);
      toast("Gagal cek status pembayaran Midtrans.");
    } finally {
  ...
  const handleUploadProof = async (file: File) => {
    ...
    try {
      ...
      const json = await res.json();
      if (json.status === "success")
        setProofUrl(`${IMG_API}?_id=${json.data.id}`);
  ...
  const itemName =
    ...
    bookingData.wahana?.name ??
    "Paket";
  const itemPath = bookingData.tourId
    ? `/tours/${bookingData.tourId}`
    : bookingData.destinationId
      ? `/destinations/${bookingData.destinationId}`
      : bookingData.wahanaId
        ? `/wahana/${bookingData.wahanaId}`
        : "#";
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span
                    style={{
                      fontSize: 13,
                      fontWeight: s.num === activeStep ? 700 : 400,
                      color:
                        s.num <= activeStep ? "white" : "rgba(255,255,255,0.5)",
                      fontFamily: "var(--font-body)",
                      whiteSpace: "nowrap",
                    }}
                  >
                ...
                {i < STEPS.length - 1 && (
                  <div
                    style={{
                      ...
                      height: 2,
                      background:
                        s.num < activeStep ? "white" : "rgba(255,255,255,0.2)",
                      margin: "0 12px",
                      flexShrink: 0,
                    }}
                  />
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            {step === "done" && (
              ...
              >
                ...
                >
                  Terima kasih! Pembayaran Anda sudah terkonfirmasi. Detail
                  pemesanan akan kami kirim ke email{" "}
                ...
                {(paidPayment || currentPayment) && (
                  ...
                  >
                    ...
                    >
                      {(paidPayment ?? currentPayment)?.referenceCode}
                    </div>
                  </div>
                )}
                <div
            ...
            {step === "select" && (
              <div className="pay-section">
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        {midtransClientKey
                          ? "Lanjutkan ke Midtrans untuk memilih bank transfer atau QRIS"
                  ...
                  {midtransClientKey ? (
                    ...
                    >
                      ...
                      >
                        Pembayaran akan diproses lewat <strong>Midtrans</strong>
                        . Anda tidak perlu memilih bank di sini, pilihan bank
                        atau e-wallet tersedia langsung di halaman Midtrans.
                      </p>
                    </div>
                  ) : (
                    <>
                      ...
                      >
                        {Object.keys(grouped).map((type) => (
                          ...
                          >
                            <span>{typeEmojis[type]}</span> {typeLabels[type]}{" "}
                      ...
                      {!selectedType && (
                        ...
                        >
                          Pilih kategori metode pembayaran di atas untuk melihat
                          daftar.
                        </div>
                      )}
                    </>
                ...
                <button
                  ...
                  disabled={
                    midtransClientKey
                      ? paymentMethods.length === 0 || submitting
                      : !selectedMethodId || submitting
                  }
                  className="btn-primary"
                  style={{
                    width: "100%",
                    ...
                    fontSize: 16,
                    background:
                      (midtransClientKey && paymentMethods.length === 0) ||
                      (!midtransClientKey && !selectedMethodId)
                        ? "#ccc"
            ...
            >
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                {bookingData.tour && (
                  ...
                  >
                    ⏱ {bookingData.tour.durationDays} hari
                  </span>
                )}
              </div>
              ...
              >
                {(
                  [
                    {
                      l: "Nama",
                      v: `${bookingData.firstName} ${bookingData.lastName}`,
                    },
                    ...
                    bookingData.tour
                      ...
                      : {
                          l: "Tanggal Kunjungan",
                          v: fmtDate(bookingData.startDate),
                        },
                    bookingData.tour
                    ...
                    {
                      l: "Peserta",
                      v: `${bookingData.adults} dewasa, ${bookingData.children} anak`,
                    },
              ...
              >
                {(() => {
                  const s = step === "done" ? "PAID" : "PENDING";
              ...
              >
                💡 Setelah pembayaran dikonfirmasi, tim kami akan mengirimkan
                e-ticket dan detail perjalanan ke email Anda dalam 1×24 jam.
              </div>
            </div>
          </aside>
        </div>
