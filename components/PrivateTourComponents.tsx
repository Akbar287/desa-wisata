...
export default function PrivateTourComponents({ tours, testimonials, vipPerks }: { tours: Tour[], testimonials: Testimonial[], vipPerks: { emoji: string; title: string; desc: string }[] }) {
    const fadeUp = {
    ...
    return (
        <main>
            <section className="relative pt-20 min-h-[520px] flex items-center overflow-hidden">
                <Image
                    src="/assets/e50bd774-982a-4206-b5b9-3ace3c9c8f27-gobi_gallery1.jpg"
                    alt="Private Tour"
                    fill
                    className="object-cover"
                    priority
                />
                <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, rgba(27,67,50,0.7) 0%, rgba(0,0,0,0.65) 50%, rgba(27,67,50,0.85) 100%)' }} />

                {[...Array(6)].map((_, i) => (
                    <motion.div
                        key={i}
                        className="absolute w-1 h-1 rounded-full"
                        style={{ background: 'rgba(212,168,67,0.6)', left: `${15 + i * 14}%`, top: `${30 + (i % 3) * 18}%` }}
                        animate={{ y: [0, -20, 0], opacity: [0.3, 0.8, 0.3] }}
                        transition={{ duration: 3 + i * 0.5, repeat: Infinity, ease: 'easeInOut' as const, delay: i * 0.7 }}
                    />
                ))}
                ...
                <div className="relative z-2 w-full max-w-[900px] mx-auto px-6 py-16 text-center">
                    <motion.div className="flex items-center justify-center gap-2 mb-4" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                        ...
                        <span className="font-sans text-sm text-white/90">Private Tour</span>
                    </motion.div>

                    <motion.div
                        className="inline-flex items-center gap-2 px-5 py-2 rounded-full mb-5"
                        style={{ background: 'linear-gradient(135deg, rgba(212,168,67,0.2), rgba(212,168,67,0.08))', border: '1px solid rgba(212,168,67,0.3)' }}
                        initial={{ opacity: 0, scale: 0.8 }}
                        animate={{ opacity: 1, scale: 1 }}
                        transition={{ duration: 0.5, delay: 0.1 }}
                    >
                        <span className="text-lg">✨</span>
                        <span className="font-sans text-xs font-semibold text-amber-200 uppercase tracking-widest">Eksklusif & Privat</span>
                    </motion.div>
                    ...
                    >
                        Private Tour<br />
                        <span style={{ color: '#D4A843' }}>VIP Experience</span>
                    </motion.h1>
                    ...
                    >
                        Nikmati keindahan Desa Manud Jaya tanpa keramaian. Perjalanan eksklusif hanya untuk Anda dan rombongan, dengan pelayanan terbaik di setiap detil.
                    </motion.p>
                    ...
                    >
                        {[
                            { value: '100%', label: 'Privat', emoji: '🔒' },
                            { value: `${tours.length}`, label: 'Paket Tersedia', emoji: '�' },
                            { value: '24/7', label: 'Concierge', emoji: '🛎️' },
                        ].map((stat) => (
                ...
                <div className="max-w-[1100px] mx-auto">
                    <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={stagger}>
                        <motion.div variants={fadeUp} className="text-center mb-12">
                            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full font-sans text-[11px] font-semibold uppercase tracking-widest mb-4" style={{ background: 'rgba(212,168,67,0.1)', color: '#B8860B', border: '1px solid rgba(212,168,67,0.2)' }}>
                                ✨ Keistimewaan VIP
                            </span>
                            <h2 className="font-serif font-bold mb-3" style={{ fontSize: 'clamp(24px, 3vw, 36px)', color: 'var(--color-text)' }}>
                                Mengapa Memilih Private Tour?
                            </h2>
                            <p className="font-sans text-sm max-w-[480px] mx-auto leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>
                                Setiap detail dirancang khusus demi kenyamanan dan pengalaman terbaik Anda.
                            </p>
                        </motion.div>
                        ...
                        <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5" variants={stagger}>
                            {vipPerks.map((perk) => (
                                ...
                                >
                                    <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl mb-4 transition-transform duration-300 group-hover:scale-110" style={{ background: 'rgba(212,168,67,0.08)', border: '1px solid rgba(212,168,67,0.15)' }}>
                                        {perk.emoji}
                                    </div>
                ...
                <div className="max-w-[1100px] mx-auto">
                    <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-50px' }} variants={stagger}>
                        <motion.div variants={fadeUp} className="text-center mb-12">
                            ...
                            <p className="font-sans text-sm max-w-[480px] mx-auto leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>
                                Paket wisata eksklusif yang bisa Anda nikmati secara privat bersama rombongan.
                            </p>
                        </motion.div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {tours.map((tour) => (
                                ...
                                >
                                    <div className="absolute top-4 right-4 z-10 flex items-center gap-1.5 px-3 py-1.5 rounded-full font-sans text-[11px] font-bold text-white backdrop-blur-md" style={{ background: 'linear-gradient(135deg, #B8860B, #D4A843)' }}>
                                        <span>🔒</span>
                                        <span>Private</span>
                                    </div>
                                    ...
                                    <div className="p-5">
                                        ...
                                        <div className="flex flex-col gap-1.5 mb-4">
                                            {tour.highlights.slice(0, 2).map((h, i) => (
                                                <div key={i} className="flex items-center gap-2 font-sans text-xs" style={{ color: 'var(--color-text-light)' }}>
                                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#D4A843" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>
                                                    {h}
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        <div className="flex items-center gap-1.5 mb-4">
                                            <span className="flex gap-0.5">
                                                {[1, 2, 3, 4, 5].map((s) => (
                                                    <svg key={s} width="12" height="12" viewBox="0 0 24 24" fill={s <= Math.round(tour.rating) ? '#D4A843' : '#E5E7EB'}>
                                        ...
                                        <div className="pt-4 flex items-end justify-between" style={{ borderTop: '1px solid var(--color-border-subtle)' }}>
                                            ...
                                            <Link href={`/tours/${tour.id}`}>
                                                <button className="py-2.5 px-5 rounded-full font-sans text-xs font-bold text-white border-none cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-lg" style={{ background: 'linear-gradient(135deg, #B8860B, #D4A843)' }}>
                                                    Lihat Detail
                                                </button>
                                            </Link>
                                        </div>
                    ...
                    <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger}>
                        <motion.div variants={fadeUp} className="text-center mb-12">
                            ...
                            <p className="font-sans text-sm max-w-[420px] mx-auto leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>
                                Empat langkah mudah menuju pengalaman VIP Anda.
                            </p>
                        </motion.div>

                        <div className="flex flex-col gap-0">
                            {[
                                { step: '01', emoji: '💬', title: 'Konsultasi', desc: 'Hubungi kami untuk mendiskusikan preferensi, tanggal, dan kebutuhan khusus Anda.' },
                                { step: '02', emoji: '📋', title: 'Itinerari Custom', desc: 'Kami menyusun itinerari eksklusif sesuai keinginan, termasuk akomodasi dan aktivitas.' },
                                { step: '03', emoji: '✅', title: 'Konfirmasi & DP', desc: 'Setujui rencana perjalanan dan lakukan pembayaran DP untuk mengamankan jadwal.' },
                                { step: '04', emoji: '🌟', title: 'Nikmati Perjalanan', desc: 'Tim kami siap menyambut dan menemani Anda dari awal hingga akhir perjalanan.' },
                            ].map((s, i) => (
                                <motion.div key={s.step} variants={fadeUp} className="flex gap-5 items-start">
                                    <div className="flex flex-col items-center shrink-0">
                                        <div className="w-12 h-12 rounded-full flex items-center justify-center font-sans text-sm font-bold text-white" style={{ background: 'linear-gradient(135deg, #B8860B, #D4A843)' }}>
                                            {s.emoji}
                                        </div>
                                        {i < 3 && <div className="w-px h-12 my-1" style={{ background: 'linear-gradient(to bottom, #D4A843, transparent)' }} />}
                                    </div>
                                    <div className={`pb-${i < 3 ? '8' : '0'} pt-1`}>
                                        <span className="font-sans text-[11px] font-bold uppercase tracking-widest block mb-1" style={{ color: '#B8860B' }}>Langkah {s.step}</span>
                            ...
                            {testimonials.map((t) => (
                                ...
                                >
                                    <div className="flex gap-0.5 mb-4">
                                        {[1, 2, 3, 4, 5].map((s) => (
                                            <svg key={s} width="14" height="14" viewBox="0 0 24 24" fill={s <= t.rating ? '#D4A843' : '#E5E7EB'}>
                                                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                                            </svg>
                                        ))}
                                    </div>
                                    ...
                                    <div className="flex items-center gap-3 pt-4" style={{ borderTop: '1px solid var(--color-border-subtle)' }}>
                                        ...
                                        <div>
                                            ...
                                            <div className="font-sans text-[11px]" style={{ color: '#B8860B' }}>{t.role}</div>
                                        </div>
                                    </div>
                                </motion.div>
                            ))}
            ...
            {/* <section className="relative overflow-hidden py-20 px-6">
                <Image
                    src="/assets/9b4e5aa3-5ec7-4c73-b146-e0f45b9eff94-mistakes_Gallery_road.jpg"
                    alt="Private CTA"
                    fill
                    className="object-cover"
                />
                <div className="absolute inset-0" style={{ background: 'linear-gradient(to right, rgba(27,67,50,0.92) 0%, rgba(45,106,79,0.85) 50%, rgba(27,67,50,0.88) 100%)' }} />
                ...
                >
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-5" style={{ background: 'rgba(212,168,67,0.15)', border: '1px solid rgba(212,168,67,0.3)' }}>
                        <span>👑</span>
                        <span className="font-sans text-[11px] font-semibold text-amber-200 uppercase tracking-widest">Eksklusif Terbatas</span>
                    </div>
                    <h2 className="font-serif text-2xl md:text-3xl font-bold text-white mb-3" style={{ textShadow: '0 2px 15px rgba(0,0,0,0.3)' }}>
                        Siap untuk Pengalaman VIP?
                    </h2>
                    <p className="font-sans text-sm text-white/75 mb-7 leading-relaxed max-w-[440px] mx-auto">
                        Jadwal terbatas setiap bulan. Hubungi kami sekarang untuk konsultasi gratis dan susun itinerari impian Anda.
                    </p>
                    <div className="flex flex-wrap justify-center gap-3">
                        <Link
                            ...
                            style={{ background: 'linear-gradient(135deg, #B8860B, #D4A843)', color: 'white' }}
                        >
                            Konsultasi Gratis
                        ...
                        >
                            <div className="relative h-[240px] overflow-hidden rounded-t-3xl">
                                ...
                                <div className="absolute top-4 left-5">
                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full font-sans text-[11px] font-bold text-white" style={{ background: 'linear-gradient(135deg, #B8860B, #D4A843)' }}>
                                        🔒 Private Tour
                                    </span>
                                </div>
                            ...
                            <div className="p-7 space-y-6">
                                    ...
                                    <div className="flex flex-col gap-2.5">
                                        {selectedTour.highlights.map((h, i) => (
                                            <div key={i} className="flex items-center gap-2.5 font-sans text-sm" style={{ color: 'var(--color-text-light)' }}>
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#D4A843" strokeWidth="3"><polyline points="20 6 9 17 4 12" /></svg>
                                                {h}
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div>
                                    <h3 className="font-serif text-base font-bold mb-3 flex items-center gap-2" style={{ color: 'var(--color-text)' }}>
                                        <span>�</span> Keuntungan Private
                                    </h3>
                                    <div className="flex flex-wrap gap-2">
                                        {['Pemandu privat', 'Transportasi eksklusif', 'Jadwal fleksibel', 'Dokumentasi foto', 'Tanpa keramaian'].map((inc, i) => (
                                            <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full font-sans text-xs font-medium" style={{ background: 'rgba(212,168,67,0.08)', color: '#8B6914', border: '1px solid rgba(212,168,67,0.2)' }}>
                                                ✓ {inc}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <div className="flex items-center gap-2">
                                    <span className="flex gap-0.5">
                                        {[1, 2, 3, 4, 5].map((s) => (
                                            <svg key={s} width="15" height="15" viewBox="0 0 24 24" fill={s <= Math.round(selectedTour.rating) ? '#D4A843' : '#E5E7EB'}>
                                                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                                            </svg>
                                        ))}
                                    </span>
                                ...
                                <div className="rounded-xl p-5" style={{ background: 'var(--color-cream)', border: '1px solid rgba(212,168,67,0.2)' }}>
                                    <div className="font-sans text-[11px] uppercase tracking-wider font-semibold mb-1" style={{ color: 'var(--color-text-muted)' }}>Harga Private Tour</div>
                                ...
                                <div className="flex flex-col sm:flex-row gap-3">
                                    <Link href={`/tours/${selectedTour.id}`} className="flex-1 no-underline">
                                        <button className="w-full py-3.5 rounded-full font-sans text-sm font-bold text-white border-none cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-lg" style={{ background: 'linear-gradient(135deg, #B8860B, #D4A843)' }}>
