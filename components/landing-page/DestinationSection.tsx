...
export default function DestinationSection({ destinations }: { destinations: DestItem[] }) {
    ...
    return (
        <section id="destinations" className="section-earthy-warm py-24 overflow-hidden relative">
            ...
            <div className="max-w-[1320px] mx-auto px-6 relative z-3">
                ...
                >
                    ...
                    <motion.h2
                        ...
                        style={{ fontSize: "clamp(28px, 4vw, 48px)", color: "var(--color-text)" }}
                    >
                        Destinasi Wisata
                    </motion.h2>
                    <motion.p
                        ...
                        style={{ color: "var(--color-text-muted)", fontSize: "clamp(14px, 1.1vw, 16px)" }}
                    >
                    ...
                    {filters.map(f => (
                        <motion.button
                            ...
                            style={{
                                ...
                                boxShadow: activeFilter === f ? "0 4px 15px rgba(45,106,79,0.25)" : "none",
                            }}
                            ...
                            >
                                ...
                                <div className="relative h-[200px] overflow-hidden shrink-0">
                                    {dest.imageBanner ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center" style={{ background: "var(--color-cream)" }}>
                                            <span className="text-4xl">🏞️</span>
                                        </div>
                                    )}
                                    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ background: "linear-gradient(to top, rgba(0,0,0,0.35) 0%, transparent 50%)" }} />
                                ...
                                <div className="p-5 pb-6 flex flex-col flex-1">
                                        ...
                                        <div className="flex gap-1.5 flex-wrap mb-4">
                                            {dest.facilities.slice(0, 3).map(f => (
                                                <span
                                                    ...
                                                    style={{ background: "var(--color-cream)", color: "var(--color-text-light)", border: "1px solid var(--color-border-subtle)" }}
                                                >
                                                    {f}
                                                </span>
                                            ))}
                                    ...
                                    >
                                        <span className="inline-flex items-center gap-1.5 font-sans text-[13px]" style={{ color: "var(--color-text-muted)" }}>
                                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>
                                            {dest.jamBuka} - {dest.jamTutup}
                                        </span>
                                        <span className="inline-flex items-center gap-1.5 font-sans text-[13px] font-bold" style={{ color: "var(--color-primary)" }}>
                ...
                {filtered.length === 0 && (
                    ...
                    >
                        ...
                        <p className="font-serif text-lg font-bold mb-2" style={{ color: "var(--color-text)" }}>Tidak ada destinasi</p>
                        <p className="font-sans text-sm" style={{ color: "var(--color-text-muted)" }}>Coba pilih kategori lain</p>
                    </motion.div>
                )}
